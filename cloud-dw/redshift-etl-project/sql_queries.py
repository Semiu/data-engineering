...
CREATE TABLE user_table (
    user_table_id INT IDENTITY(0,1) PRIMARY KEY,
    user_id INTEGER,
    first_name TEXT,
    last_name TEXT,
    gender TEXT,
...
CREATE TABLE artist_table (
    artist_table_id INT IDENTITY(0,1) PRIMARY KEY,
    artist_id VARCHAR(MAX) NOT NULL,
    artist_name VARCHAR(MAX),
    artist_location TEXT,
    artist_latitude DOUBLE PRECISION,
...
CREATE TABLE time_table (
    time_table_id INT IDENTITY(0,1) PRIMARY KEY,
...
staging_events_copy = (
    """copy staging_events_table from '{}' credentials 'aws_iam_role={}' format as JSON 'auto' region 'us-west-2';
"""
).format(LOG_DATA, ARN)

staging_songs_copy = (
    """copy staging_songs_table from '{}' credentials 'aws_iam_role={}' format as JSON 'auto' region 'us-west-2';
"""
...
JOIN staging_songs_table sts ON (ste.artist = sts.artist_name) AND (ste.song = sts.title);
"""
